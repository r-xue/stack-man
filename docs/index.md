...
5. **Dual Container Output**: Environments are natively packaged as both Apptainer `.sif` files (for HPCs) and Docker OCI images (for cloud platforms) — generally, use Docker for fast, iterative local builds via layer caching, and use Apptainer for deploying secure, highly-compressed monolithic files to supercomputers.
